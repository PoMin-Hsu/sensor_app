...
public class WiFiDataManager {
	...
	public ArrayList<Long> unixTimestamp = new ArrayList<Long>(); //bill
	...
	private final BroadcastReceiver cycleWifiReceiver = new BroadcastReceiver() {
		...
		public void onReceive(Context context, Intent intent) {
			...
			wifiManager.startScan();
			unixTimestamp.add(System.currentTimeMillis()/1000L); //bill
			//Log.i("moon", "unixTimestamp:" + unixTimestamp);
	...
	public void endCollecting(MainActivity activity) {
		...
		// 然后存储数据到文件
		new FileManager(activity).saveData();
